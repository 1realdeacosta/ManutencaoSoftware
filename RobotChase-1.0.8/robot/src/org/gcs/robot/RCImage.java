...
/**
 * The RCImage class supplies images to the {@link org.gcs.robot.RCView RCView}
 * view. Each set is a single instance containing the images used in play. There
 * are three sets, each having a common image type: png, gif or jpg. The images
 * are assumed to have a uniform aspect ratio, defined in
 * {@link org.gcs.robot.RCTile RCTile}.
 *
 * @see RCTile
...
public final class RCImage {
	...
	private static Image getImage(String name) {
		...
		JOptionPane.showMessageDialog(null, messages, "I/O Error", JOptionPane.ERROR_MESSAGE);
		System.exit(0);
		return null;
	}
	...
	public static RCImage getNextSet() {
		if (".png".equals(current))
			current = ".gif";
		else if (".gif".equals(current))
			current = ".jpg";
		else if (".jpg".equals(current))
			current = ".png";
