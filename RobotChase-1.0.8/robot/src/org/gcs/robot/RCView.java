...
/**
 * The RCView class is the main view of the game board. It observes the game
 * model and determines what to draw on each square, delegating the actual
 * drawing to {@link org.gcs.robot.RCTile RCTile}. The view also implements
 * MouseListener and MouseMotionListener, as an alternative to keyboard input.
 * If animation is enabled, a timer moves the player as far as possible at a
 * rate determined by DELAY.
 *
 * @see RCModel
 * @see RCTile
 * @serial exclude
 ...
 */
public class RCView extends JPanel implements ActionListener, MouseListener, MouseMotionListener, Observer {
	...
	/**
	 * Resize the game and view. Resize the game board, create a conformal view and
	 * initialize it.
	 * 
	...
	public int getWidthInTiles() {
		return max(MINTILE, tileCol(getWidth()));
	}

	/** Return the view height in tiles. */
	public int getHeightInTiles() {
		return max(MINTILE, tileRow(getHeight()));
	}
	...
	public void update(Observable model, Object arg) {
		...
		tiles[p.y][p.x].setImg(game.lost() ? RCImage.Dead : RCImage.Me);
		this.repaint();
	}
	...
	private Image select(int token) {
		...
		case BOMB:
			return RCImage.Bomb;
		case EMPTY:
			return RCImage.Blank;
		case ROBOT:
			return RCImage.Robot;
		default:
			return RCImage.Wreck;
		}
	}
	...
	public void actionPerformed(ActionEvent e) {
		if (!game.move(mouseRow, mouseCol) || game.won())
			timer.stop();
	}
