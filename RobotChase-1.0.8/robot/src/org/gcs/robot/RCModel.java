...
/**
 * The RCModel class models the game board as a two dimensional array. The array
 * contains <i>height</i> rows and <i>width</i> columns. Public constants
 * indicate the meaning of an array element; a value of two or more represents a
 * collision. The player's position is a Point representing the current row
 * &amp; column.
 *
 * The player's moves are handled by the {@link #move(int) move()} methods.
 * Overloaded versions of {@link #move(int, int) move()} are used for mouse
 * input.
 *
...
public class RCModel extends Observable {
	...
	/**
	 * Resize the game board array. Notification is deferred to give observers an
	 * opportunity to adjust. The observer may call initLevel() when ready.
	 * 
	 * @param width  board width measured in tiles
	 * @param height board height measured in tiles
	 */
	...
	public void initLevel() {
		...
		liveRobots = botCount;
		setObject(botCount, ROBOT);

		// now POSTS
		int postCount = randomInt(min(width, height) + 5, 2 * (max(width, height) + 5));
		setObject(postCount, POST);
		// and BOMBS
		int bombCount = randomInt(width / 5, width / 5 + 2);
		setObject(bombCount, BOMB);
		// finally, our hero
		do {
			pRow = randomRow();
			pCol = randomCol();
	...
	public void setObject(int count, int objectType) {
		while (count > 0) {
			int row = randomRow();
			int col = randomCol();
			if (board[row][col] == EMPTY) {
				board[row][col] = objectType;
				count--;
			}
		}
	}
	...
	/**
	 * Return the player's current position as a <code>java.awt.Point</code>. This
	 * point represents a two-dimensional array index: <code>point.x</code> holds
	 * the column index and <code>point.y</code> holds the row index.
	 *
	 * @return the player's board position in row, column order
	 */
	public Point getPosition() {
		return new Point(pCol, pRow);
	}

	/** Return true if the player has died; false otherwise. */
	public boolean lost() {
		return board[pRow][pCol] > EMPTY;
	}

	/** Return true if the player has won; false otherwise. */
	public boolean won() {
		return countBots(board) == 0;
	}
