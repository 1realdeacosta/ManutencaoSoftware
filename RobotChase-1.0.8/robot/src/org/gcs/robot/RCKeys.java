 ...
 */
public class RCKeys extends JDialog implements ActionListener, FocusListener, KeyListener {

	private EnumMap<Key, Integer> savedKeys = new EnumMap<Key, Integer>(Key.class);
	private HashMap<JTextField, Key> keyMap = new HashMap<JTextField, Key>(savedKeys.size() * 4 / 3);
	...
	private JPanel genTopPanel() {
		...
		JLabel label = new JLabel("Click or tab to select a field, then press the desired key.", icon, JLabel.LEADING);
	...
	private JPanel genCenterPanel() {
		...
		p2.setBorder(title);
		for (Key k : EnumSet.range(Key.NW, Key.NE))
			p2.add(genKeyPanel(k));
		for (Key k : EnumSet.range(Key.WEST, Key.EAST))
			p2.add(genKeyPanel(k));
		for (Key k : EnumSet.range(Key.SW, Key.SE))
			p2.add(genKeyPanel(k));
	...
	/**
	 * Highlight any duplicate keys. Duplicates are not forbidden, but they should
	 * be apparent, as only the first will be used. The algorithm compares each text
	 * field to every other, except itself, and accumulates fields with duplicate
	 * keys. The text is black by default, while duplicates are red.
	 */
	...
	public boolean verificaTextField(JTextField t1, JTextField t2) {
		int k = keyMap.get(t1).getKeyCode();
		int j = keyMap.get(t2).getKeyCode();
		return t1 != t2 && k == j;
	}
